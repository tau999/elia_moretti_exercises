...
void Equilibration (int iblk){
	if(iblk ==1){

		for (int i=0; i<equilibration_time; i++)	Move();
...
//g(r)
		...
		for (int i = igofr; i<n_props; i+=1){

      double r = bin_size*(i-igofr);	
			stima_gofr = blk_av[i]/(blk_norm*(rho * npart * (4. * M_PI / 3.) * (pow(r + bin_size, 3) - pow(r, 3))));;
			glob_av[i+igofr] += stima_gofr;
    	glob_av2[i+igofr] += stima_gofr*stima_gofr;
			err_gave=Error(glob_av[i+igofr],glob_av2[i+igofr],iblk);
			Gofr << setw(wd) << stima_gofr;
			
			if (iblk==nblk){
				Gave << setw(wd) << r <<  setw(wd) << glob_av[i+igofr]/(double)iblk << setw(wd) << err_gave << endl;
			}
		}
		Gofr << setw(wd) << endl;
